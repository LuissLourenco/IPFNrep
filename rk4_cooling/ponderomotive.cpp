...
void ponderomotive(){
  ...
  double* gama_f = new double[n_data];
  ...
  for(counter = 0; counter < 6; counter++){
    ...
    while(r <= r_max){
      ...
      p_y_f[i] = sol[n-1][4];
      p_y_max[i] = 0;
      gama_f[i] = sqrt(1. + sol[n-1][3]*sol[n-1][3] + sol[n-1][4]*sol[n-1][4] + sol[n-1][5]*sol[n-1][5]);
      for(int j = 0; j < n; j++){
    ...
    fprintf(file, "r\ty_f\tp_y_max\tp_x_f\tp_y_f\tgama_f\tw0=%lf|lambda=1|E0=%lf|time=100|kdamp=%.10e|px=-2000"
