...
void mood(string file_in, string plot_out, string file_out){
	...
	if(sqrt(Y*Y+Z*Z).getMax().val() > 10) return;
...
int main(int argc, char** argv){
	...
	sprintf(directory, "../Data_p0l0_0/");
	files = list_dir(directory, &n_files);

	for(int i = 0; i < n_files; i++){
		if(i % 40 == 0) continue;
		sprintf(plot, "plots_p0l0/plot%s.png", files[i].substr(string(directory).size(), 8).c_str());
		sprintf(log, "logs_p0l0/log%s.png", files[i].substr(string(directory).size(), 8).c_str());
		//mood(files[i], plot, log);
		sprintf(cmd, "gnome-terminal -- bash -ic './test %s %s %s'; sleep 1", files[i].c_str(), plot, log);
		system(cmd);
		cout << files[i] << endl;
	}
	...
	/*
	mood("../Data_px-10_a50_dt0.003/Out00063.txt", "testplot1.png", "logout1.txt");
	mood("../Data_px-10_a50_dt0.003/Out00121.txt", "testplot2.png", "logout2.txt");
	mood("../Data_px-10_a50_dt0.003/Out00181.txt", "testplot3.png", "logout3.txt");
	mood("../Data_px-10_a50_dt0.003/Out00651.txt", "testplot4.png", "logout4.txt");
	*/
}