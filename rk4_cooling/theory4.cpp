...
complex<double> upl(double r, double phi, double z){
	...
	res *= exp(-r*r/wz/wz);
	double aux = -k*r*r*z/2./(z*z+zr*zr)-(double)l*phi+(2.*(double)p+(double)abs(l)+1.)*atan(z/zr);
	res *= exp(imu*aux);

	return res;
}
complex<double> Alg(double r, double phi, double z){
	return upl(r,phi,z)*exp(imu*(w*t-k*z));
}
complex<double> electric_field_luis(double x, double y, double z){
	complex<double> res;
	double r = sqrt(y*y+z*z);
	double phi = atan2(z,y);
	res = -imu*w*upl(r,phi,x)*exp(imu*(w*t-k*x));
	return res;
}
complex<double> magnetic_field_luis(double x, double y, double z){
	...
	if(l != 0) res *= 1.+(double)abs(l);
	if(p != 0){
		complex<double> upl_gay(1,0);
		upl_gay *= Eo*w0/wz;
		upl_gay *= pow(r*sqrt(2.)/wz, abs(l));
		upl_gay *= assoc_laguerre(abs(p)-1, abs(l)+1, 2.*r*r/wz/wz); //<<<===
		upl_gay *= exp(-r*r/wz/wz);
		double aux = -k*r*r*x/2./(x*x+zr*zr)-l*phi+(2.*(double)p+(double)abs(l)+1.)*atan(x/zr);
		upl_gay *= exp(imu*aux);

		res += 4*r*r*w0*w0*x/zr/zr/wz/wz/wz/wz*upl_gay;
	}
	double aux = -k*r*r/2.*(zr*zr-x*x)/(x*x+zr*zr)/(x*x+zr*zr)+(2.*(double)p+(double)abs(l)+1.)*zr/(x*x+zr*zr)-k;
